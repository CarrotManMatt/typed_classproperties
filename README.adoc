...
:_url-peps: https://peps.python.org
...
:url-ty-home: https://astral.sh/ty
:url-ty: {url-ty-home}
...
:labelled-url-pep767: {url-pep767}[PEP767]
...
This package makes use of some reasonably advanced Python functionality that could be considered a hack.
The appropriate solution to correctly annotate the types matching the runtime behaviour of classproperties is explained in {labelled-url-pep767}.
This PEP, however, is still in draft and not yet accepted; therefore, not all type checkers are supported.

While we will attempt to fix type annotation bugs in this project for alternative type-checkers, our focus is on the following officially supported type checkersfootnote:[The list of officially supported type checkers is subject to change between any minor version release]:

* {labelled-url-mypy}
* {labelled-url-ty} &le; 0.0.59footnote:[Support for {labelled-url-ty} is capped to below https://github.com/astral-sh/ty/releases/tag/0.0.60[0.0.60], which began to find type-errors with the hacks used in this project]
